...
real_price = real_price[90:180]

dataset_val = dataset_val[['Open','Close']]
inputs  = dataset_val[:len(dataset_val)-95].values #Formato fila
inputs = sc.transform(inputs) #Reescalamos los datos (el mínimo y el máximo se han obtenido del sc anterior.)

X_val = []
X_val2 = []
for i in range(0, 90):
    X_val.append(inputs[i:i+90,0])
    X_val2.append(inputs[i:i+90,1])
...
predicted_price = np.append(predicted_price, ([[0]]*90), axis = 1)
predicted_price = sc.inverse_transform(predicted_price)
